...
describe('addToCart', () => {
  ...
  it('should add an item to a new anonymous cart', async () => {
    ...
    expect(mockCookies.set).toHaveBeenCalledWith('cartId', 'new-cart-id', expect.any(Object));
  ...
  it('should add an item to an existing anonymous cart', async () => {
    ...
    (prisma.cart.findUnique as vi.Mock).mockResolvedValue({ id: 'existing-cart-id' });
    (prisma.cartItem.findUnique as vi.Mock).mockResolvedValue(null);

    await addToCart(mockCookies, 'product-1', 1);

    expect(prisma.cart.findUnique).toHaveBeenCalledWith({ where: { id: 'existing-cart-id' } });
    expect(prisma.cartItem.create).toHaveBeenCalledWith({
      data: {
        productId: 'product-1',
  ...
  it('should update an item in an existing anonymous cart', async () => {
    (mockCookies.get as vi.Mock).mockReturnValue('existing-cart-id');
    (prisma.cart.findUnique as vi.Mock).mockResolvedValue({ id: 'existing-cart-id' });
    (prisma.cartItem.findUnique as vi.Mock).mockResolvedValue({ id: 'item-1', productId: 'product-1', cartId: 'existing-cart-id', quantity: 1 });
