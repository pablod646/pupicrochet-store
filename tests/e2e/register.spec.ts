...
test.describe("Registration Flow", () => {
  test("should allow a user to register successfully", async ({ page }) => {
    ...
    await expect(
      page.locator("h2", { hasText: "Inicia sesión en tu cuenta" }),
    ).toBeVisible();
  });

  test("should display error messages for invalid registration data", async ({
    page,
  }) => {
    await page.goto("/register");
    ...
    await page.click('button[type="submit"]');
    await expect(
      page.locator("p.text-red-600", { hasText: "Passwords don't match" }),
    ).toBeVisible();
  });
});
