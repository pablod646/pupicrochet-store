<script lang="ts">
  ...
  let selectedCategoryForSubcategory: string | undefined;
  ...
  $: if (form?.message) {
    ...
    if (form.success) {
      newCategoryName = '';
      newSubcategoryName = '';
      selectedCategoryForSubcategory = undefined;
      invalidateAll();
    }
  }
  ...
  const handleDeleteSubcategory = () => {
    ...
    return async ({ result }) => {
      if (result.type === 'success') {
        message = result.data?.message || 'Subcategoría eliminada exitosamente!';
        messageType = 'success';
        await invalidateAll();
      } else if (result.type === 'failure') {
        message = result.data?.message || 'Fallo al eliminar la subcategoría.';
        messageType = 'error';
      } else if (result.type === 'error') {
        message = 'Ocurrió un error inesperado al eliminar la subcategoría.';
        messageType = 'error';
      }
    };
  };
  ...
  <div class="bg-white p-6 rounded-lg shadow">
    ...
    <form method="POST" action="?/createCategoryOrSubcategory" use:enhance>
      ...
      <div class="mb-4">
        ...
        <select id="parentCategoryId" name="parentCategoryId" bind:value={selectedCategoryForSubcategory} class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            ...
            <ul class="ml-4 list-disc list-inside">
              {#each category.subcategories as subcategory}
                <li class="flex justify-between items-center">
                  ...
                  <form method="POST" action="?/deleteSubcategory" use:enhance={handleDeleteSubcategory}>
